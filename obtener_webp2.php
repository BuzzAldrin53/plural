...
<head>
 ...
 <body>
 	...
 	<style type="text/css">
		...
		$sz = " SELECT count(*) FROM `xml_productos` WHERE `fotowebp` = '1' ";
		...
		$sz = " SELECT count(*) FROM `xml_productos` WHERE `fotowebp` = '0' ";
		$resulz = $mysqli->query($sz);
		while ($rowz = $resulz->fetch_row()){
			$conta=$rowz[0];
		}
		echo "<h1>Pendientes: ".$conta."</h1>";
						...
						$sz = " SELECT * FROM `xml_productos` WHERE `fotowebp` = '0' ORDER BY id_producto ASC LIMIT 2";
						$resulz = $mysqli->query($sz);
						while ($rowz = $resulz->fetch_row()){
							...
							$precio = number_format($rowz[30]);
								...
								while ($rowa = $resula->fetch_row()){
			                        ...
			                        $ruta_para_guardar = './xmlfotos/fotos4/';
									...
									$origen = imagecreatefromjpeg($imagen_alta);
									list($ancho, $alto) = getimagesize($imagen_alta);
